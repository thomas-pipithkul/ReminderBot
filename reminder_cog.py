...
class ReminderCog(commands.Cog):
    ...
    async def parse(self, ctx, *, dt_str):
        ...
        if ret is not None:
            ...
            await ctx.send(parsed_datetime.strftime("%m/%d/%Y, %H:%M:%S %Z"))
        else:
    ...
    async def now(self, ctx):
        ...
        await ctx.send(f'now utc: {now_utc.strftime("%m/%d/%Y, %H:%M:%S %Z")}\nnow local: {now_local.strftime("%m/%d/%Y, %H:%M:%S %Z")}')
